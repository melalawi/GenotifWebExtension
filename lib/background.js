...
function BackendManager(browserAdaptor) {
    ...
    this.lookupGene = function(geneID, callback) {
        var lookupURL = QUERY_URL.replace('GENE_ID', geneID),
            selected = extensionSettings.settings.selectedOrganism;
        
        // Instantiate set if it doesn't exist yet
        extensionSettings.cachedResults[selected] = extensionSettings.cachedResults[selected] || {};

        if (extensionSettings.cachedResults[selected][geneID]) {
            callback(extensionSettings.cachedResults[selected][geneID]);
        } else {
            adaptor.XMLRequest(lookupURL, function(response){
                extensionSettings.cachedResults[selected][geneID] = response;
                callback(response);
            });
        }
    };
