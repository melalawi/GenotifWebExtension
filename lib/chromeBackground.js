...
var manager,
    ...
    scriptFile: 'js/chromeContentScript.js',
    ...
    sendMessage: function(tab, message, data) {
        chrome.tabs.sendMessage(tab.id, {message: message, data: data});
    },
    ...
    messageListener: function(messageHandler) {
        ...
        listenerCallback = function(request, sender, sendResponse) {
            if (messageHandler && sender.tab) {
                messageHandler(sender.tab, request.message, request.data, sendResponse);
            }
        };
...
function initialize() {
    manager = GTE_BACKGROUND.generateManager(Deferred, adaptor);
    manager.initialize();
}
